...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (!m_QueueUpdateScheduler.empty())
    {
        ...
        for (uint8 i = 0; i < scheduled.size(); i++)
        {
            ...
            m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(diff, bgTypeId, bracket_id);
        }
    }
}

void BattlegroundMgr::BuildBattlegroundStatusPacket(WorldPacket* data, Battleground* bg, uint8 QueueSlot, uint8 StatusID, uint32 Time1, uint32 Time2, uint8 uiFrame)
{
    ...
    *data << uint32(QueueSlot);                             // queue id (0...1) - player can be in 2 queues in time
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "BattlegroundMgr::BuildBattlegroundStatusPacket: bg instanceID %u, TypeID %u.", bg->GetClientInstanceID(), bg->GetTypeID());
    *data << uint8(0x2);
...
{                                                      // last check on 3.0.3
    ...
    *data << uint8(0);                                   // type (battleground)
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId bgTypeId, PvPDifficultyEntry const* bracketEntry)
{
    // get the template BG
...
void BattlegroundMgr::ScheduleQueueUpdate(BattlegroundQueueTypeId bgQueueTypeId, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id)
{
    ...
    uint64 schedule_id = ((bgQueueTypeId << 16) | (bgTypeId << 8) | bracket_id);
