...
public:
    ...
    static bool HandleReloadCreatureTemplateCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (Tokens::const_iterator itr = entries.begin(); itr != entries.end(); ++itr)
        {
            ...
            cInfo->faction_A          = fields[15].GetUInt16();
            cInfo->faction_H          = fields[16].GetUInt16();
            ...
            cInfo->minrangedmg        = fields[37].GetFloat();
            cInfo->maxrangedmg        = fields[38].GetFloat();
            cInfo->rangedattackpower  = fields[39].GetUInt16();
            cInfo->type               = fields[40].GetUInt8();
            cInfo->type_flags         = fields[41].GetUInt32();
            cInfo->lootid             = fields[42].GetUInt32();
            cInfo->pickpocketLootId   = fields[43].GetUInt32();
            cInfo->SkinLootId         = fields[44].GetUInt32();

            for (uint8 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
                cInfo->resistance[i] = fields[45 + i -1].GetUInt16();

            cInfo->spells[0]          = fields[51].GetUInt32();
            cInfo->spells[1]          = fields[52].GetUInt32();
            cInfo->spells[2]          = fields[53].GetUInt32();
            cInfo->spells[3]          = fields[54].GetUInt32();
            cInfo->spells[4]          = fields[55].GetUInt32();
            cInfo->spells[5]          = fields[56].GetUInt32();
            cInfo->spells[6]          = fields[57].GetUInt32();
            cInfo->spells[7]          = fields[58].GetUInt32();
            cInfo->PetSpellDataId     = fields[59].GetUInt32();
            cInfo->mingold            = fields[60].GetUInt32();
            cInfo->maxgold            = fields[61].GetUInt32();
            cInfo->AIName             = fields[62].GetString();
            cInfo->MovementType       = fields[63].GetUInt8();
            cInfo->InhabitType        = fields[64].GetUInt8();
            cInfo->HoverHeight        = fields[65].GetFloat();
            cInfo->ModHealth          = fields[66].GetFloat();
            cInfo->ModMana            = fields[67].GetFloat();
            cInfo->ModArmor           = fields[68].GetFloat();
            cInfo->RacialLeader       = fields[69].GetBool();
            cInfo->questItems[0]      = fields[70].GetUInt32();
            cInfo->questItems[1]      = fields[71].GetUInt32();
            cInfo->questItems[2]      = fields[72].GetUInt32();
            cInfo->questItems[3]      = fields[73].GetUInt32();
            cInfo->questItems[4]      = fields[74].GetUInt32();
            cInfo->questItems[5]      = fields[75].GetUInt32();
            cInfo->movementId         = fields[76].GetUInt32();
            cInfo->RegenHealth        = fields[77].GetBool();
            cInfo->equipmentId        = fields[78].GetUInt32();
            cInfo->MechanicImmuneMask = fields[79].GetUInt32();
            cInfo->flags_extra        = fields[80].GetUInt32();
            cInfo->ScriptID           = sObjectMgr->GetScriptId(fields[81].GetCString());
